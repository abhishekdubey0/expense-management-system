...
public class KafkaConfig {
    ...
    // Auto-create topics with replication
    @Bean
    public NewTopic expenseSubmittedTopic() {
        return TopicBuilder.name("expense-submitted").partitions(3).replicas(1).build();
    }

    @Bean
    public NewTopic expenseStatusChangedTopic() {
        return TopicBuilder.name("expense-status-changed").partitions(3).replicas(1).build();
    }
}
